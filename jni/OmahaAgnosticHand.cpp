...
#include "Card.h"
#include "oh10maxordering.h"
#include "oh6maxordering.h"
#include "o810maxordering.h"
#include "o86maxordering.h"
...
int OmahaAgnosticHand::Parse(const char* handText, StdDeck_CardMask deadCards)
{
    ...
    while (*p != '\0' && *p != '/') {
        ...
        else if (*p != '\0' && NULL != strchr("AaKkQqJjTt98765432BbRrFfMmZzLlNnYyXx", *p)) {
            if (*p != '\0' && NULL != strchr("AaKkQqJjTt98765432", *p)) {
                if (p[1] == '-') {
    ...
    if (*p == '/') {
        while (*p == '/') {
            p++;
            if (*p != '\0' && NULL != strchr("Nn", *p)) {
                p++;
                if (*p != '\0' && NULL != strchr("Pp", *p)) {
                    m_isNoPair = true;
                }
                else if (*p != '\0' && NULL != strchr("Tt", *p)) {
                    m_isNoTrips = true;
                }
                else if (*p != '\0' && NULL != strchr("Qq", *p)) {
                    m_isNoQuads = true;
                }
                else {
            ...
            else if (*p != '\0' && NULL != strchr("Oo", *p)) {
                p++;
                if (*p != '\0' && NULL != strchr("Pp", *p)) {
                    m_isOnePair = true;
                }
                else if (*p != '\0' && NULL != strchr("Ss", *p)) {
                    m_isOneSuited = true;
                }
            }
            else if (*p != '\0' && NULL != strchr("Tt", *p)) {
            ...
            else if (*p != '\0' && NULL != strchr("Mm", *p)) {
                p++;
                if (*p != '\0' && NULL != strchr("Tt", *p)) {
                    m_isMonotone = true;
                }
                else {
                ...
                else if (*p != '\0' && NULL != strchr("Aa", *p)) {
                    m_isSuitedAce = true;
                }
                else if (*p != '\0' && NULL != strchr("Nn", *p)) {
                    p++;
                    if (*p != '\0' && NULL != strchr("Aa", *p)) {
  ...
  error:
    printf("Error parsing %s, parsing at %s: %d\n", handText, p, p-handText+1);
    return 0; // failure
}
...
bool OmahaAgnosticHand::IsRandomHand(const char *handText)
{
    if (strlen(handText) == 4 &&
            (handText[0] == 'X' || handText[0] == 'x') &&
            (handText[1] == 'X' || handText[1] == 'x') &&
            (handText[2] == 'X' || handText[2] == 'x') &&
            (handText[3] == 'X' || handText[3] == 'x')) {
        return true; // valid
    }
    return false;
}

bool OmahaAgnosticHand::IsPercentRange(const char* handText, double &lowerBound, double &upperBound)
{
    char *percent, *p;
...
int OmahaAgnosticHand::InstantiatePercentRange(const char* handText, StdDeck_CardMask deadCards, vector<StdDeck_CardMask>& specificHands)
{
    if ((m_isPercent = IsPercentRange(handText, m_lowerBound, m_upperBound))) {
        int size = sizeof(OMAHA_10_MAX_ORDERING)/sizeof(const char *);
